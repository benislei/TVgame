...
当前接收端 APK 面向 Android 11+（API 30+）电视。阶段 2 的目标是优先验证高画质、高帧率、低输入延迟的游戏体验，不再为 Android 6/7/8 做兼容妥协。

如果电视系统低于 Android 11，安装时可能提示 SDK 版本过低，这是预期结果。低版本电视后续考虑通过独立电视盒子或硬件接收端路线解决。
...
可以把 `app-debug.apk` 复制到 Android 11+ 电视，通过电视上的文件管理器安装。安装前请确认设备允许安装未知来源应用。
