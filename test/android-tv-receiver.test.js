...
test('DiscoveryBroadcaster advertises the Android receiver on the LAN', () => {
  ...
  assert.match(source, /BROADCAST_INTERVAL_MS\s*=\s*2000/);
  assert.match(source, /Thread\.sleep\(BROADCAST_INTERVAL_MS\)/);
  assert.match(source, /if\s*\(\s*running\s*\)\s*\{\s*return;\s*\}/);
  assert.match(source, /public\s+synchronized\s+void\s+stop\(\)\s*\{\s*running\s*=\s*false;[\s\S]*?thread\.interrupt\(\);[\s\S]*?thread\s*=\s*null;/);
  assert.match(source, /catch\s*\(\s*Exception\s+\w+\s*\)\s*\{[\s\S]*?Discovery is best-effort/);
  assert.match(source, /StringBuilder/);
  assert.match(source, /for\s*\(\s*int\s+i\s*=\s*0;\s*i\s*<\s*text\.length\(\);/);
  assert.match(source, /case\s+'\\\\'/);
  assert.equal(source.includes("case '\"':"), true);
  assert.equal(source.includes("case '\\b':"), true);
  assert.equal(source.includes("case '\\f':"), true);
  assert.equal(source.includes("case '\\n':"), true);
  assert.equal(source.includes("case '\\r':"), true);
  assert.equal(source.includes("case '\\t':"), true);
  assert.match(source, /value\s*<\s*0x20/);
  assert.match(source, /"\\\\u%04x"/);
...
test('MainActivity shows device diagnostics and restarts stalled video receiver', () => {
  ...
  assert.match(source, /Build\.MANUFACTURER/);
  assert.match(source, /Build\.MODEL/);
  assert.match(source, /Build\.HARDWARE/);
  assert.match(source, /containsIgnoreCase\(.*amlogic/s);
  assert.match(source, /activeSurface/);
  assert.match(source, /monitorVideoHealth\(\)/);
  assert.match(source, /VIDEO_STALL_RESTART_MS\s*=\s*1200/);
  assert.match(source, /VIDEO_STALL_MIN_PACKETS\s*=\s*60/);
...
test('video and audio receivers use required ports, codecs and stats', () => {
  ...
  assert.match(video, /MAX_PENDING_ACCESS_UNITS\s*=\s*2/);
