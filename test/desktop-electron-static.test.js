...
test('renderer exposes repair progress and blocks stream start until every environment card is normal', () => {
  ...
  assert.match(html, /id="repairProgressIntro"/);
  assert.match(html, /id="repairElapsedText"/);
  assert.match(html, /id="repairCurrentTitle"/);
  assert.match(html, /id="repairProgressPercent"/);
  assert.match(html, /id="repairProgressBar"/);
  assert.match(html, /id="repairCurrentStep"/);
  assert.match(html, /id="repairDownloadSpeed"/);
  assert.match(html, /id="repairEta"/);
  assert.match(html, /id="repairFileSize"/);
  assert.match(html, /id="repairDownloader"/);
  assert.match(html, /id="diagnosticRecentStatusText"/);
  assert.match(html, /环境处理进度/);
  assert.match(html, /修复进度/);
  assert.match(html, /安装阶段反馈/);
  assert.doesNotMatch(html, /id="repairProgressList"/);
  assert.doesNotMatch(html, /仍在继续/);
  ...
  assert.match(appJs, /function renderDiagnosticRepairProgress\(/);
  assert.match(appJs, /function inferRepairMetadata\(/);
  assert.match(appJs, /function repairElapsed\(/);
  ...
  assert.match(styles, /\.repair-progress-panel--diagnostics/);
  assert.match(styles, /\.repair-metric-grid/);
  assert.match(styles, /\.repair-stage-card/);
